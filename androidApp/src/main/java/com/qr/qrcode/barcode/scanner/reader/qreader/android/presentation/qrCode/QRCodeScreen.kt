...
import com.qr.qrcode.barcode.scanner.reader.qreader.android.presentation.QRActivity
...
fun QRCodeScreen(
    ...
    if (!isEditable) {
        LaunchedEffect(id, encoded, state.isSaved) {
            if (!state.isSaved) {
                viewModel.onEvent(
                    QRCodeEvent.Insert(
                        id = id,
    ...
    LaunchedEffect(true) {
        (context.findActivity() as? QRActivity)?.showAds(hasSubscription)
    }
